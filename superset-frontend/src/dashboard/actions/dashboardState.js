...
const storeDashboardMetadata = async (id, metadata, getState) => {
  const { dash_edit_perm } = getState().dashboardInfo;

  // Skip saving if user doesn't have edit permissions (e.g., anonymous users)
  if (!dash_edit_perm) {
    console.log(
      '[storeDashboardMetadata] Skipping save - user lacks edit permissions (dash_edit_perm=false)',
    );
    return;
  }
...
export const persistDashboardLabelsColor = () => async (dispatch, getState) => {
  ...
  if (labelsColorMapMustSync || sharedLabelsColorsMustSync) {
    ...
    storeDashboardMetadata(id, metadata, getState);
  }
};
